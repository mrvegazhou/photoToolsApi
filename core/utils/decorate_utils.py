...
def single_schedule_task(redis_client=None, lock_key=None, lock_timeoout=None):
    def wrap(func):
        def inner(*args, **kwargs):
            with RedisLock(redis_client=redis_client, lock_key=lock_key, lock_timeoout=lock_timeoout) as r:
                if r.is_acquired:
                    print("当前节点获取任务：{}!".format(lock_key))
                    try:
                    ...
                    print("当前节点未获取任务：{}".format(lock_key))
                    return
        return inner
    return wrap
