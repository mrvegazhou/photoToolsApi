...
# token缓存前缀
TOKEN_PREFIX = "admin_user_token_"
# token缓存时间
...
def need_login(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        # 解析token
        # key = current_app.config.get("secret_key", SECRET_KEY)
        try:
        ...
        if cached_token:
            # token不同时，把cached_token解析出来比对哪里有变化，以便给出精准提示
            if not compare_digest(cached_token, token):
                try:
        ...
        g.admin_user_login_time = create_time
...
def clear_token(user_id):
    ...
    """
    return cache.delete("{0}{1}".format(TOKEN_PREFIX, user_id))


def get_token(user_id):
    return cache.get("{0}{1}".format(TOKEN_PREFIX, user_id))
...
if __name__ == "__main__":
    token = create_token(1, "2222")
    s1 = URLSafeSerializer("secret-key")
    tmp = s1.dumps(".eJwNjLmVxUAIwHpx7IB7oJZ9G8AA_ZfwHSmR9Ifv49sqHic5eMVriBWjm4tz9eTzPhYdw6o21awV2EVA0jmacwpdkyoAjpVh5Xdk9NtjtxYTg_xM3zDDZCA3SVmSo58d4Wv3qK9_ucfFSbOkrJGzS00p9rwG4gAvmn1wYPj_AYBfMXg.8iPagZNPQC9kqqj4HXJbNiXRP38")

    s2 = URLSafeSerializer(SECRET_KEY)
    print(s2.loads(token))
